...
SimpleSwitch::transmit_thread() {
  while (1) {
    ...
    //send dpdk packet                
    dpdk_api_send_packet(packet->get_egress_port(), packet->data(), packet->get_data_size());
...
SimpleSwitch::egress_thread(size_t worker_id) {
  ...
  while (1) {
    ...
    if (packet == nullptr) break;
    //test
    dpdk_output_buffer_0.push_front(std::move(packet));
    dpdk_output_buffer_0.pop_back(&packet);
