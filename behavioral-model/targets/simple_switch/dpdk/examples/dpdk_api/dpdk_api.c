...
static void
print_packets(struct rte_mbuf *m)
{
  uint16_t packet_len = 0;
  char *s_output = NULL;
  int i;
  packet_len = rte_ctrlmbuf_len(m);
  s_output = rte_pktmbuf_mtod(m, char*);
  //print packet data
  printf("\n receive packet, length: %d \n", packet_len);
  for (i = 0; i < packet_len; i++) {
    printf("%02x ", (uint8_t)s_output[i]);
  }
  printf("\n======================\n");
}
...
int dpdk_api_send_packet(uint32_t port, char* data, uint16_t packet_len)
{
  ...
  m_send = rte_pktmbuf_alloc(mbuf_pool);
  if(m_send != NULL) {
    s_data_output = rte_pktmbuf_append(m_send, packet_len);
    //copy data
    memcpy(s_data_output, data, sizeof(char)*packet_len);
    rte_eth_tx_burst(port, 0, &m_send, 1);
    //free packet
...
int lcore_main(void *arg)
{
  ...
  while (!force_quit) {
    for (port = 0; port < nb_ports; port++) {
      ...
      //print all packets data
      //for (buf = 0; buf < nb_rx; buf++) {
        //print_packets(bufs[buf]);
      //}
      // 交换mac地址
      mac_swap(bufs, nb_rx);
      ...
      for (buf = 0; buf < nb_rx; ++buf) {
        ...
        //allocate memory
        //s_data_output = (char *)malloc(packet_len*sizeof(char));
        
        //send packet
        dpdk_api_send_packet(port, s_data_buffer, packet_len);
        //free memory
        //free(s_data_output);
      }
